...
var GarminService = function(sessionId, sessionToken, callback) {
  ...
  this._validateSession(callback);
};
...
GarminService.prototype.getUpdate = function(callback) {
  ...
  if(!this._sessionValidated) {
    this.emit('error', new Error('Session invalidated'));
    return;
  }
  ...
  request(url, (function(err, res, body) {
    ...
    if(err || res.statusCode != 200) {
      this.emit('error', new Error('Issue while getting tracking log'));
      return;
    }
    ...
    try {
      ...
      this.emit('error', new Error('Invalid tracking log response'));
    }
